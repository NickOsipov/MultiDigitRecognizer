...
1. Запускаем `run.sh` в терминале. Соберется docker-image `mdrecognizer` и в интерактивном режиме поднимется docker-container `mdr_container`. После остановки контейнер удаляется.  
```bash
bash run.sh
```
